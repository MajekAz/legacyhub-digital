...
The suite uses mocked fetch and a local Apps Script service harness. It never accesses a live Google Sheet. Tests cover Zod normalisation/validation, consent, honeypot, API origin/content type/size/rate errors, authenticated CRM requests and safe redirects, malformed or failed responses, timeout, central IDs, counter continuity after deletion, idempotency, shared-secret rejection, counter collisions, follow-up deduplication/recovery, schema drift, spreadsheet injection, email failure isolation, scoring, attribution consent/persistence, analytics gates, WhatsApp configuration, page content, forms and metadata.
