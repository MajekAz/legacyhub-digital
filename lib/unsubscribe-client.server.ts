...
const responseSchema = z.union([
  z.object({ ok: z.literal(true), status: z.enum(['unsubscribed', 'already_unsubscribed']) }),
  z.object({ ok: z.literal(false), error: z.literal('invalid_token') }),
]);
export async function unsubscribeEmail(token: string) {
  ...
  try {
    let res = await fetch(url, {
      ...
      body: JSON.stringify({ secret, action: 'unsubscribe', data: { token } }),
